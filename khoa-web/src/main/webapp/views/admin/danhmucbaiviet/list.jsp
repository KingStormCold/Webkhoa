...
<c:url var="requestUrl" value="/admin-danhmucbaiviet-list.html"/>
<html>
<head>
    <meta content="text/html; charset=utf-8" http-equiv="Content-Type" />
    <title><fmt:message key="label.danhmuc.home" bundle="${lang}"/> </title>
</head>
<body>
                                            ...
                                            <div class="widget-main">
                                                <div class="form-horizontal">
                                                    <div class="form-group">
                                                        <label class="col-sm-2 control-label"><fmt:message key="label.danhmuc.name" bundle="${lang}"/></label>
                                                        <div class="col-sm-8">
                                                            <div class="fg-line">
                                                                <input type="text" value="${item.pojo.tenDanhMucBaiViet}" class="form-control input-sm" name="pojo.tenDanhMucBaiViet"/>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                    ...
                                    <div class="table-btn-controls">
                                        <div class="pull-right tableTools-container">
                                            <div class="dt-buttons btn-overlap btn-group">
                                                <c:url var="addUrl" value="/admin-danhmucbaiviet-edit.html">
                                ...
                                <fmt:bundle basename="ApplicationResources">
                                    <display:table id="tableList" name="items.listResult" partialList="true" size="${items.totalItems}"
                                        ...
                                        <display:column title="<fieldset class='form-group'>
                                            ...
                                            <fieldset>
                                                <input type="checkbox" name="checkList" id="checkboz_${tableList.danhMucBaiVietId}" value="${tableList.danhMucBaiVietId}" />
                                            </fieldset>
                                        </display:column>
                                        <display:column property="tenDanhMucBaiViet" titleKey="label.danhmuc.name" sortable="true" sortName="tenDanhMucBaiViet"/>
                                        <display:column titleKey="label.danhmuc.cha" sortable="true" sortName="chaDanhMucBaiViet">
                                        <c:forEach items="${items.listResult}" var="list">
                                            <c:if test="${list.danhMucBaiVietId eq tableList.chaDanhMucBaiViet}">
                                                ${list.tenDanhMucBaiViet}
                                            </c:if>
                                            <c:if test="${list.danhMucBaiVietId eq 0}">
                                                Không
                                            </c:if>
                                        </c:forEach>
                                        </display:column>
                                        <display:column property="status" titleKey="label.danhmuc.status" sortable="true" sortName="status"/>
                                        <display:column headerClass="col-actions" titleKey="label.action">
                                            <c:url var="editUrl" value="/admin-danhmucbaiviet-edit.html">
                                                <c:param name="urlType" value="url_edit"/>
                                                <c:param name="pojo.danhMucBaiVietId" value="${tableList.danhMucBaiVietId}"/>
                                            </c:url>
